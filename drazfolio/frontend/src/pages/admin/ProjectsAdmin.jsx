...
export default function ProjectsAdmin() {
  ...
  const normalizeCover = (raw) => {
    ...
    if (!v) return '';
    // already a full URL (include blob/data URLs from in-browser uploads)
  ...
  const uploadFile = async (file) => {
    ...
    try {
      setUploading(true);
      setProgress(0);
      // prefer Cloudinary unsigned upload if configured, otherwise mock backend
      const { uploadImage } = await import('../../services/upload.js');
      const res = await uploadImage(file, (ev) => { if (ev.total) setProgress(Math.round((ev.loaded / ev.total) * 100)); });
      setForm((s) => ({ ...s, coverImage: res.url }));
      setPreviewKey(Date.now());
