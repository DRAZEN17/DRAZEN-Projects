        ...
        <nav className="mt-6 flex flex-col gap-1">
          {links.map((l) => (
            <NavLink key={l.to} to={l.to} end={l.end}
                     className={({ isActive }) => `px-3 py-2 rounded-lg text-sm transition ${isActive ? 'bg-white/10 text-white' : 'text-muted hover:text-white hover:bg-white/5'}`}>
              {l.label}
            </NavLink>
          ))}
        </nav>
        <button onClick={async () => { await logout(); navigate('/login'); }}
                className="mt-6 text-sm text-muted hover:text-neon-magenta">Logout</button>
      </aside>
